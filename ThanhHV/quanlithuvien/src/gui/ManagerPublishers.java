...
public class ManagerPublishers extends JPanel {
	public ManagerPublishers() {
		...
		JPanel addPublishers = new AddPublishers();
		wrapPanel1.add(Box.createVerticalGlue());
		wrapPanel1.add(addPublishers);
		wrapPanel1.add(Box.createVerticalGlue());
		JComponent panel1 = makeTextPanel("Panel #1", wrapPanel1);
		tabbedPane.addTab("Thêm Nhà Xuất Bản", icon, panel1, "Add Publishers");
		...
		JPanel editPublishers = new EditPublishers();
		wrapPanel2.add(Box.createVerticalGlue());
		wrapPanel2.add(editPublishers);
		wrapPanel2.add(Box.createVerticalGlue());
		// panel2.setPreferredSize(new Dimension(400,300));
		JComponent panel2 = makeTextPanel("Panel #2", wrapPanel2);
		tabbedPane.addTab("Cập nhật nhà xuất bản", icon, panel2, "Edit Publishers");
	...
	protected static ImageIcon createImageIcon(String path) {
		java.net.URL imgURL = ManagerPublishers.class.getResource(path);
		if (imgURL != null) {
	...
	private static void createAndShowGUI() {
		...
		JFrame frame = new JFrame("ManagerPublishers");
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		// Add content to the window.
		frame.add(new ManagerPublishers(), BorderLayout.CENTER);
