...
public class ManagerAuthors extends JPanel {
	public ManagerAuthors() {
		...
		JPanel addAuthors = new AddAuthors();
		wrapPanel1.add(Box.createVerticalGlue());
		wrapPanel1.add(addAuthors);
		wrapPanel1.add(Box.createVerticalGlue());
		JComponent panel1 = makeTextPanel("Panel #1", wrapPanel1);
		tabbedPane.addTab("Thêm tác giả", icon, panel1, "Add Authors");
		...
		JPanel editAuthors = new EditAuthors();
		wrapPanel2.add(Box.createVerticalGlue());
		wrapPanel2.add(editAuthors);
		wrapPanel2.add(Box.createVerticalGlue());
		// panel2.setPreferredSize(new Dimension(400,300));
		JComponent panel2 = makeTextPanel("Panel #2", wrapPanel2);
		tabbedPane.addTab("Cập nhật tác giả", icon, panel2, "Edit Authors");
	...
	protected static ImageIcon createImageIcon(String path) {
		java.net.URL imgURL = ManagerAuthors.class.getResource(path);
		if (imgURL != null) {
	...
	private static void createAndShowGUI() {
		...
		JFrame frame = new JFrame("ManagerAuthors");
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		// Add content to the window.
		frame.add(new ManagerAuthors(), BorderLayout.CENTER);
