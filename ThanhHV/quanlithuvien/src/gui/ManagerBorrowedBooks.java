...
public class ManagerBorrowedBooks extends JPanel {
	public ManagerBorrowedBooks() {
		...
		JPanel borrowBooks = new BorrowBooks();
		wrapPanel1.add(Box.createVerticalGlue());
		wrapPanel1.add(borrowBooks);
		wrapPanel1.add(Box.createVerticalGlue());
		JComponent panel1 = makeTextPanel("Panel #1", wrapPanel1);
		tabbedPane.addTab("Mượn sách", icon, panel1, "Borrow Books");
		...
		JPanel returnBooks = new ReturnBooks();
		wrapPanel2.add(Box.createVerticalGlue());
		wrapPanel2.add(returnBooks);
		wrapPanel2.add(Box.createVerticalGlue());
		// panel2.setPreferredSize(new Dimension(400,300));
		JComponent panel2 = makeTextPanel("Panel #2", wrapPanel2);
		tabbedPane.addTab("Trả sách", icon, panel2, "Return Books");
	...
	protected static ImageIcon createImageIcon(String path) {
		java.net.URL imgURL = ManagerBorrowedBooks.class.getResource(path);
		if (imgURL != null) {
	...
	private static void createAndShowGUI() {
		...
		JFrame frame = new JFrame("ManagerBorrowedBooks");
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

		// Add content to the window.
		frame.add(new ManagerBorrowedBooks(), BorderLayout.CENTER);
